...
- Re-run with `--approve-outline`:
  ...
  - by default, immediately refreshes `tutorial_final.html`,
    `tutorial_final.pdf`, and `render_manifest.json` through the downstream
    renderer
  - records unresolved editorial issues as warnings in `tutorial_manifest.json`
  - reports `status: "partial"` and returns a non-zero exit code if Markdown
    publish succeeds but the downstream PDF render fails
...
- `--agents-dir`
- `--skills-dir`
- `--skip-render` to publish Markdown without attempting the automatic render
